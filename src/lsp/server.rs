...
fn should_run_legacy_msvc_compile_database_fallback(config: &EffectiveConfig) -> bool {
    config.preset == "msvc-cmake-ninja" && config.build.system == "cmake"
}
...
fn discover_optional_msvc_environment(
    ...
    if config.toolchain.name != "msvc" && config.preset != "msvc-cmake-ninja" {
        return None;
    }
...
fn write_generated_tasks(
    ...
    let contents = generate_cpp_tasks_json(config, default_shell_for_current_platform())?;

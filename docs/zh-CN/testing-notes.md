...
1. **CMake LSP 激活**
   ...
   - 预期: 看到 "cpp-toolkit-neocmake" LSP 启动
...
3. **PATH 缺失处理**
   ...
   - 预期: 日志显示 "neocmakelsp not found in PATH"
   - 预期: CMake LSP 启动失败，并提示用户安装 neocmakelsp 后加入 PATH
   - 扩展不会自动下载 GitHub Release
...
- 扩展编译为 `target/wasm32-unknown-unknown/release/cpp_toolkit.wasm`
- 需要将此 WASM 文件复制到 Zed 扩展目录进行测试
- Zed 扩展目录通常位于 `%ZED_USER_EXTENSIONS_DIR%`
- neocmakelsp 只使用 PATH 中的二进制；PATH 中不存在时不自动下载。
